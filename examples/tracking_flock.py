...
class BlimpTracking(Device):
  def setup(self):
    ...
    self.random_counter =5 #var to slightly change velocity vector every 5 timesteps
  ...
  def weight(self, a, dont_increase = False): #don't increase if we want the robot to slow down when approaching the blimp
    ...
    w = float(sum)*2.0 #scale factor of array
    if w<1.0 and dont_increase and sum<0.2: w = 1.0 #don't increase when sum is small
    return a/w
  def step(self):
    a = self.sum_hood_plus(self.nbr(1))
    #if a>=0: self.red = 255
    ...
    if self.sense0:
        self.known_locs[self.id] = (self.coord, self.time)
    ...
    for (k, v) in self.field.items():
      if v:
        (coord, tracking_start_time, delta, offsets, known_locs) = v
        if not self.is_my_coord(coord): # loop over neighbors only (redundant?)
                
          for offKey in offsets.keys():
            if self.offsets.has_key(offKey):
              if self.offsets[offKey][1] < offsets[offKey][1]: self.offsets[offKey] = offsets[offKey] #nbr offset is newer 
            else: self.offsets[offKey] = offsets[offKey] #
        
          for locKey in known_locs.keys():
            if self.known_locs.has_key(locKey):
              if self.known_locs[locKey][1] < known_locs[locKey][1]: self.known_locs[locKey] = known_locs[locKey] #offsets is newer 
            else: self.known_locs[locKey] = known_locs[locKey] #
        ...
        if tracking_start_time > -1 and delta != None and self.is_close(coord + delta):
            ...
            if not self.prev_next:
              self.prev_next = True
              self.offsets[self.id] = (5*delta,self.time)
    ...
    for k in self.offsets.keys():
      offset = self.offsets[k][0]
      offset_start_time = self.offsets[k][1]
      ...
      if ((self.time - offset_start_time) > self.offset_timeout): 
        del(self.offsets[k]) #offset is old
    
    for k in self.known_locs.keys():
      known_loc = self.known_locs[k][0]
      loc_time = self.known_locs[k][1]
      ...
      if ((self.time - self.loc_time) > self.loc_timeout): 
        del(self.known_locs[k]) #known_loc is old
    if self.known_locs == {} and self.remember_velocity != None: #head in direction of offset, since known locs are out of date
    ...
    if self.move_vector != None:
      ...
      if abs(tempX) + abs(tempY) + abs(tempZ) >= 0.2:
        self.remember_velocity = self.move_vector
      #print abs(tempX) + abs(tempY) + abs(tempZ)
    ...
    self.field = self.nbr((self.coord, self.tracking_start_time, self.delta, self.offsets, self.known_locs))
        
    self.prev_sense = self.sense0
        
#spawn_cloud(klass=CommunicationTest, robot = DifferentialWheelsRobot)
#spawn_cloud(klass=MoveTest, robot = DifferentialWheelsRobot)
spawn_cloud(klass=BlimpTracking)
