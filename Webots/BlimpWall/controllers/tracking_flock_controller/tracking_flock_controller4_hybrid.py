...
class BlimpTracking(Device):
  ...
  def step(self):
    ...
    for (k, v) in self.field.items():
      if v:
        (coord, tracking_start_time, delta, offsets, known_loc, loc_time) = v
        ...
        if known_loc != None:
          if self.known_loc == None: #use nbr's known loc since we don't have one
            ...
            self.known_loc = known_loc #take nbr's loc
            self.loc_time = loc_time #take nbr's time
    ...
    while self.offsets.keys().__len__()>3: #delete oldest keys until only have 3 offsets
      oldest_time = None
      oldest_key = None
      for k in self.offsets.keys():
        if oldest_time == None: 
          oldest_time = self.offsets[k][1]
          oldest_key = k
        elif self.offsets[k][1]< oldest_time:
          oldest_time = self.offsets[k][1]
          oldest_key = k
      del(self.offsets[oldest_key]) #delete oldest key
    ...
    self.field = self.nbr((self.coord, self.tracking_start_time, self.delta, self.offsets, self.known_loc, self.loc_time))
